...
def summarise_game_reviews(game_review_data: dict, client: OpenAI, prompts: dict, db_path: str = "game_reviews.db") -> dict:
    ...
    for i in trange(len(game_review_data)):
        ...
        game_summary = get_gpt_response(client, messages)
...
def analyse_game_review_summary(game_review_df: pd.DataFrame, client: OpenAI, prompts: dict) -> dict:
    """
    Generate an overall did_well / needs_work summary across all stored games.

    Args:
        game_review_df: DataFrame of all stored reviews as returned by
            pd.read_sql("SELECT * FROM reviews", conn).
        client: An initialised OpenAI client.
        prompts: Dict of prompt strings; must contain the key
            'go_review_summary_analyser'.

    Returns:
        Dict with 'did_well' and 'needs_work' keys.
    """
    messages = [
        {
            "role": "system",
        ...
        },
        {
            ...
            "content": f"""Here is the JSON of game review summaries: \n
                {game_review_df.drop(columns=["review_notes", "game_link"]).to_json(orient="records", indent=2)}"""
        }
    ]
